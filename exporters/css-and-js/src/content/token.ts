...
export function tokenObjectKeyName(
  ...
  // Append the theme name (merged-theme-suffix mode), re-cased to the configured style
  if (themeSuffix) {
    name = NamingHelper.codeSafeVariableName(`${name} ${themeSuffix}`, exportConfiguration.tokenNameStyle)
  }
