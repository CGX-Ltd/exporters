 ...
 * `mergedSuffixSeparateThemes`. Matched by theme name, case-insensitive. When the config list is
...
export function partitionThemesForSuffix(themes: Array<TokenTheme>): {
  ...
  const names = new Set(exportConfiguration.mergedSuffixSeparateThemes.map((n) => n.trim().toLowerCase()))
