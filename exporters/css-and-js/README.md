...
The CSS and JS Exporter is a powerful package for converting your design system data into production-ready CSS variables and/or CSS-in-JS. It facilitates a seamless transition from design to development, ensuring consistency and accuracy throughout the process. The exporter has a variety of configuration options to make sure the output always fits your codebase.

## Exporter features

This exporter package takes your design system tokens and converts them to CSS variables and/or CSS-in-JS in various ways. Here are its key features:

- **Dual output:** Emit CSS custom properties (`.css`), CSS-in-JS (`.ts`), or both in a single export — controlled by the `outputType` option.
- **Theme merging:** Optionally merge every theme's overrides into the base files, suffixing each themed variable with the theme name (e.g. `--margin--dark` / `marginDark`) so all themes can be consumed side by side. Enabled via `exportThemesAs: "mergedThemeSuffix"`.
- **Support for all Supernova token types:** Generates output for all token types, including colors, text styles, shadows, dimensions and more.
...
### Output
- **outputType:** Choose what the exporter emits — `css` (CSS variables only), `js` (CSS-in-JS only), or `both`.
...
- **Redundant prefix stripping:** Both CSS and JS names are de-duplicated by cutting to the first `core` / `semantic` segment, so a token whose group path repeats its type prefix (e.g. `border-radius-semantic-border-radius-small`) becomes `semantic-border-radius-small`. This is a no-op for tokens that contain no `core`/`semantic` anchor. The anchor list lives in [`src/utils/name-utils.ts`](src/utils/name-utils.ts).
...
- **useFallbackValues:** Include raw token values as fallbacks for CSS `var()` references (CSS output only).
- **colorPrecision:** Maximum number of decimals in colors.

### Themes
- **exportThemesAs:** Control how themes are exported:
  - `separateFiles` — a separate file/folder per theme.
  - `applyDirectly` — themes baked into the base output.
  - `mergedTheme` — one combined themed output with all themes applied.
  - `mergedThemeSuffix` — merge every theme's overrides into the base files, adding a `--name--theme` (CSS) / `nameTheme` (JS) variable for each overridden token. Base values stay unsuffixed; only overridden tokens gain a suffixed variant.
...
### CSS selectors (CSS output only)
- **cssSelector:** Selector for base token variables (e.g. `:root`).
- **themeSelector:** Selector pattern for theme files; `{theme}` is replaced with the theme name.

### Index file
- **generateIndexFile:** Generate a file with imports for all base and theme files.
- **indexFileName:** Name of the generated CSS-in-JS index file (`.ts`).
- **cssIndexFileName:** Name of the generated CSS index file (`.css`).
...
- **generateFolderIndexFiles:** Generate an `index.ts` in each theme/base folder (CSS-in-JS output only).
- **fileStructure:** Organize CSS output as separate files per token type or a single combined file (CSS output only).
...
- **propertyToWriteNameToIncludesVar:** Wrap the written name in `var()` syntax for easier copying (CSS output only).
